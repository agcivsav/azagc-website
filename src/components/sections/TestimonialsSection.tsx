...
export default function TestimonialsSection({
  ...
  const hasAny = Object.values(testimonialsByCategory).some((arr) => arr.length > 0)
      ...
      <section className={cn('bg-cream py-16', className)}>
        <div className="container-site">
          <p className="font-body text-slate text-center py-12">
            No testimonials yet. Add testimonials in Sanity Studio and assign a category to see them here.
          </p>
        </div>
      </section>
    )
  ...
  return (
    <section className={cn('bg-cream py-16', className)} aria-label="Testimonials">
      <div className="container-site">
        <div
          role="tablist"
        ...
        >
          {TESTIMONIAL_CATEGORIES.map(({ value, label }) => {
            const count = (testimonialsByCategory[value] ?? []).length
        ...
        >
          {items.length === 0 ? (
            <p className="font-body text-slate">No testimonials in this category.</p>
          ) : (
            items.map((item) => <TestimonialCard key={item._id} item={item} />)
          )}
        </div>
      </div>
    </section>
