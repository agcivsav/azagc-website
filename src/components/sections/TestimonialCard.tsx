    ...
    >
      <div className="min-w-0">
        ...
        <footer className="mt-4">
          <p id={`testimonial-name-${item._id}`} className="font-semibold text-navy">
            {item.personName}
            {item.personTitle && (
              <span className="font-normal text-slate text-sm block mt-0.5">{item.personTitle}</span>
            )}
          </p>
        </footer>
      </div>
      <div className="flex items-center justify-center gap-3 md:pt-2">
        {item.logoUrl && item.logoUrl.startsWith('http') ? (
          <div className=" flex items-center justify-center shrink-0 bg-white/50 rounded">
