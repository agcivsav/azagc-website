...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!webhookUrl) {
      console.error('[GHL] Missing GHL_WEBHOOK_URL')
      return NextResponse.json(
        { error: 'Server is missing dashboard webhook configuration' },
        { status: 500 }
      )
    }
    ...
    if (!ghlRes.ok) {
      ...
      console.error('[GHL] Failed:', errorText)
      return NextResponse.json({ error: 'Failed to send lead to dashboard' }, { status: 500 })
    }
