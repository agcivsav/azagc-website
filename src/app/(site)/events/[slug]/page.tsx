...
export async function generateMetadata({
  ...
  return {
    ...
    description:
      event.excerpt ||
      `${event.title} — AZAGC event in ${event.location || "Arizona"}.`,
  };
}
    ...
    <>
      ...
      <div className="bg-white border-b border-warm-gray">
        <div className="container-site py-3 flex items-center gap-2 text-xs font-body text-slate">
          <Link href="/" className="hover:text-navy no-underline">
            Home
          </Link>
          <span>/</span>
          <Link href="/events" className="hover:text-navy no-underline">
            Events
          </Link>
          <span>/</span>
      ...
      <section className="bg-navy py-16">
        <div className="container-site">
          <div className="flex items-start gap-6">
            ...
            <div>
              {event.category && (
                <p className="font-body font-semibold text-xs uppercase tracking-[0.15em] text-primarymb-2">
                  {event.category}
                </p>
              )}
          ...
          <div className="space-y-4">
            {event.registrationUrl && (
              <div className="bg-white border border-warm-gray p-6">
                <h3 className="font-normal text-lg text-navy mb-3">
            ...
            {event.membersOnly && (
              <div className="bg-teal p-6">
                <p className="font-body font-semibold text-xs uppercase tracking-wide text-primarymb-2">
                  Members Only
                </p>
                <p className="font-body text-white/80 text-sm mb-4">
                  This event is exclusive to AZAGC members.
                </p>
                <LeadForm
      ...
      {/* <CTABand
        title="Get Access to All AZAGC Events"
        subtitle="AZAGC members get priority registration and member-only pricing on all events."
        primaryButton={{ label: 'Join AZAGC', href: '/join' }}
        secondaryButton={{ label: 'View All Events', href: '/events' }}
      /> */}
    </>
  );
}
